...
cp -a     "$LIVE_STACK/docker-compose.yml"     "/tmp/ids01-monitoring-compose.before-zabbix"
...
if docker compose -f "$LIVE_STACK/docker-compose.yml" config >/dev/null; then
    ...
    cp       "/tmp/ids01-monitoring-compose.before-zabbix"       "$LIVE_STACK/docker-compose.yml"
